...
function handleInitSheets() {
  ...
  if (!firstCell || firstCell !== 'id') {
    queuesSheet.getRange(1, 1, 1, 9).setValues([[
      'id', 'number', 'service', 'customer_name', 'status', 'created_at', 'called_at', 'counter', 'date'
    ]]);
    // Format header row
    queuesSheet.getRange(1, 1, 1, 9)
      .setFontWeight('bold')
  ...
  if (!firstCounterCell || firstCounterCell !== 'id') {
    countersSheet.getRange(1, 1, 1, 5).setValues([[
      'id', 'name', 'service', 'last_called_number', 'last_called_at'
    ]]);
    countersSheet.getRange(1, 1, 1, 5)
      .setFontWeight('bold')
      .setBackground('#004482')
      .setFontColor('#ffffff');
  ...
  if (!firstSettingCell || firstSettingCell !== 'key') {
    ...
    settingsSheet.appendRow(['youtubeUrl',       'https://www.youtube.com/watch?v=DHua0l0Hhu4', 'URL video YouTube untuk TV Display']);
    settingsSheet.appendRow(['autoPrint',         'true',  'Otomatis print tiket di Kiosk (true/false)']);
    settingsSheet.appendRow(['ttsVoiceUri',       '',      'Voice URI untuk Text-to-Speech']);
    settingsSheet.appendRow(['ttsPitch',          '1',     'Intonasi suara TTS (0-2)']);
    settingsSheet.appendRow(['ttsRate',           '0.8',   'Kecepatan bicara TTS (0.5-2)']);
    settingsSheet.appendRow(['videoVolume',       '100',   'Volume video normal saat tidak ada pemanggilan (0-100)']);
    settingsSheet.appendRow(['videoVolumeDucked', '15',    'Volume video saat TTS sedang membacakan antrian (0-50)']);
  }
  
  return jsonOut({ success: true, message: "Semua sheet berhasil diinisialisasi dengan header." });
}
...
function handleCreateQueue(request) {
  ...
  if (!sheet) return jsonOut({ error: "Sheet 'queues' tidak ditemukan. Jalankan init_sheets terlebih dahulu." });
  ...
  // Find last number for today and this service
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    if (rowDate) {
      var rDateStr = Utilities.formatDate(new Date(rowDate), Session.getScriptTimeZone(), "yyyy-MM-dd");
      if (rDateStr === dateStr && rowService === service) {
  ...
  sheet.appendRow([
    id, formattedNum, service, customerName,
    'waiting', new Date(), '', '', dateStr
  ]);
  
  return jsonOut({ number: formattedNum, status: 'waiting', customer_name: customerName });
}
...
function handleCallQueue(request) {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    var rDateStr = "";
    if (rowDate) {
      rDateStr = Utilities.formatDate(new Date(rowDate), Session.getScriptTimeZone(), "yyyy-MM-dd");
    }
    
    if (rDateStr === dateStr && data[i][Q_SERVICE] === service && data[i][Q_STATUS] === 'waiting') {
  ...
  if (rowToCall > -1) {
    ...
    return jsonOut({ id: foundId, number: foundNum, status: 'called', customer_name: foundCust || '' });
  } else {
    return jsonOut({ error: "Tidak ada antrian menunggu untuk layanan " + service });
  }
}
...
function handleListQueue(serviceFilter) {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    if (rDateStr === dateStr && data[i][Q_STATUS] === 'waiting') {
      if (!serviceFilter || data[i][Q_SERVICE] === serviceFilter) {
        result.push({
          ...
          created_at:    data[i][Q_CREATED_AT] ? new Date(data[i][Q_CREATED_AT]).toISOString() : ''
        });
      }
    }
  }
...
function handleDisplay() {
  var countersSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('counters');
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    if (name) {
      displayData[name] = {
        number:  lastNum  || "--",
        service: service  || "",
        called_at: lastAt ? new Date(lastAt).toISOString() : ""
      };
    }
  }
...
function updateCounterLastCalled(counterName, service, number) {
  ...
  if (!found) {
    sheet.appendRow([Utilities.getUuid(), counterName, service, number, new Date()]);
  }
}
