...
import { getWaitingQueues, callNextQueue, getConfig } from '../api';
...
export default function AdminPage() {
  ...
  const handleCall = async (_skipId?: string) => {
    ...
    try {
      ...
      const serviceName = service || (queues.length > 0 ? queues[0].service : 'CS');
      const counterName = serviceName === 'CS' ? 'Loket Customer Service' : serviceName === 'PLN' ? 'Loket PLN Mobile Experience' : 'Loket Customer Care';
      const res = await callNextQueue(serviceName, counterName);
      
      const numberCalled = res.number || queues[0].number;
      setLastCalled({
        number: numberCalled,
        service: queues.find(q => q.number === res.number)?.service || serviceName,
        counter: counterName
      });
  ...
  const handleRecall = () => {
    ...
    speakQueue(lastCalled.number, lastCalled.counter || 'Loket Customer Service', ttsConfig);
  };
    ...
    <div className="px-10 py-8 flex flex-col gap-8 h-full">
        ...
        <header className="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          <div>
            ...
            <p className="text-base text-slate-500 mt-1">Manage active customer queues and service flows.</p>
          </div>
          ...
          <div className="xl:col-span-2 bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col items-center justify-center p-12 relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col items-center">
              <div className="inline-flex items-center gap-2 bg-slate-100 px-4 py-1.5 rounded-full border border-slate-200 mb-6">
                ...
                <span className="text-sm font-medium text-slate-600 uppercase tracking-wider">Sedang Dipanggil</span>
              </div>
              ...
              <p className="text-xl text-slate-500 mt-4">
                {lastCalled ? `${lastCalled.service === 'CS' ? 'Customer Service' : lastCalled.service === 'PLN' ? 'PLN Mobile Experience' : lastCalled.service === 'CC' ? 'Customer Care' : lastCalled.service}` : 'Belum ada antrian'}
              </p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex-1 flex flex-col min-h-[300px]">
          <div className="p-6 border-b border-slate-200 flex justify-between items-center bg-slate-50">
            <h3 className="text-2xl font-semibold text-[#191c21]">Daftar Antrian</h3>
            <div className="flex items-center gap-4">
              <button onClick={fetchQueues} className="text-slate-500 hover:text-[#005BAC] transition-colors p-2">
          ...
          <div className="overflow-x-auto flex-1">
            <table className="w-full text-left border-collapse">
              <thead className="bg-white border-b border-slate-200 sticky top-0">
                <tr>
                  <th className="py-4 px-6 text-sm text-slate-500 font-semibold">Nomor Antrian</th>
                  <th className="py-4 px-6 text-sm text-slate-500 font-semibold">Jenis Layanan</th>
                  <th className="py-4 px-6 text-sm text-slate-500 font-semibold">Nama Pelanggan</th>
                  <th className="py-4 px-6 text-sm text-slate-500 font-semibold">Status</th>
                  <th className="py-4 px-6 text-sm text-slate-500 font-semibold">Waktu Ambil</th>
                </tr>
              </thead>
              <tbody className="text-base text-[#191c21] divide-y divide-slate-100">
                {queues.length > 0 ? (
                  queues.map((q, idx) => {
                    const timeStr = q.created_at ? new Date(q.created_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) + ' WIB' : '--:-- WIB';
                    return (
                      <tr key={idx} className="hover:bg-slate-50 transition-colors">
                        <td className="py-4 px-6 font-semibold text-[#004482]">{q.number}</td>
                        <td className="py-4 px-6">{q.service === 'CS' ? 'Customer Service' : q.service === 'PLN' ? 'PLN Mobile Experience' : 'Customer Care'}</td>
                        <td className="py-4 px-6 text-slate-600">{q.customer_name || <span className="text-slate-300 italic text-sm">—</span>}</td>
